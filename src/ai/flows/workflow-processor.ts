...
2.  **Identificação de Credenciais:**
    ...
    *   Retorne uma lista de objetos, cada um contendo 'platform', 'credential' e 'value'.
  ...
  async (input) => {
    ...
    if (credentials && credentials.length > 0) {
      credentials.forEach((cred) => {
        // Ensure we don't save credentials with empty/null/NA values
        if (cred.value && cred.value.trim() !== '' && cred.value.toLowerCase() !== 'n/a') {
            saveCredential({
              ...cred,
